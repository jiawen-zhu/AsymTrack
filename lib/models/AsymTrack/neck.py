...
class NECK_FPN(nn.Module):
    def __init__(self, num_x, input_dim, hidden_dim, output_dim, stride, num_layers, backbone_embed_dim, BN=False):
        ...
        backbone_embed_dim = [dim*2 for dim in backbone_embed_dim]
        ...
        if BN:
            self.layers = nn.ModuleList(nn.Sequential(nn.ConvTranspose2d(n, k, (s, s), (s, s)), nn.BatchNorm2d(k))
                                        for n, k, s in
                                        zip(backbone_embed_dim[::-1][0:-2], backbone_embed_dim[::-1][1:], st))
        else:
            self.layers = nn.ModuleList(nn.Sequential(nn.ConvTranspose2d(n, k, (s, s), (s, s)))
                                        for n, k, s in
                                        zip(backbone_embed_dim[::-1][0:-2], backbone_embed_dim[::-1][1:], st))

        self.proj1 = nn.Linear(backbone_embed_dim[1], output_dim)
        self.stride_total = stride ** num_layers
    ...
    def forward(self, xz):
        ...
        x = multiscale_features[-1]
        for i, layer in enumerate(self.layers):
            x = layer(x)
            x = x + multiscale_features[-2 - i]
            if i < self.num_layers - 1:
                x = F.relu(x)

        B, C, Len, _ = x.shape
        x = x.view(B, C, Len * Len).permute(2, 0, 1).contiguous()
        x = self.proj1(x).transpose(0, 1)
...
class NECK_FB(nn.Module):
    ...
    def __init__(self, num_x, input_dim, hidden_dim, output_dim, stride, num_layers, backbone_embed_dim, BN=False):
        ...
        if backbone_embed_dim is None:
            self.backbone_embed_dim = backbone_embed_dim
            backbone_embed_dim = [128, 256, 256]
            st = [1, 2]
        else:
            self.backbone_embed_dim = backbone_embed_dim
            st = [stride] * (num_layers)
        ...
        if BN:
            self.layers = nn.ModuleList(nn.Sequential(nn.ConvTranspose2d(n, k, (s, s), (s, s)), nn.BatchNorm2d(k))
                                        for n, k, s in
                                        zip(backbone_embed_dim[::-1][0:-1], backbone_embed_dim[::-1][1:], st))
        else:
            self.layers = nn.ModuleList(nn.Sequential(nn.ConvTranspose2d(n, k, (s, s), (s, s)))
                                        for n, k, s in
                                        zip(backbone_embed_dim[::-1][0:-1], backbone_embed_dim[::-1][1:], st))
        self.proj1 = nn.Linear(backbone_embed_dim[0], output_dim)
