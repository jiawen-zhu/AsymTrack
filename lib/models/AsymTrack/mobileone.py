...
class MobileOneBlock(nn.Module):
    ...
    def _get_kernel_bias(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.rbr_scale is not None:
            ...
            # Pad scale branch kernel to match conv branch kernel size. 把1x1卷积填充为3x3
    ...
    def _fuse_bn_tensor(self, branch) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        beta = torch.zeros(kernel.size(0))
        return kernel, beta
...
class MBoneBlock(nn.Module):
    ...
    def __init__(self,
        ...
        super(MBoneBlock, self).__init__()
        self.inference_mode = inference_mode

        self.in_channels = in_channels
        self.out_channels = out_channels

        self.conv3x3 = MobileOneBlock(in_channels=self.in_channels,
                                         out_channels=self.out_channels,
                                         kernel_size=3,
                                         stride=1,
                                         padding=1,
                                         ...
                                         use_se=False,
                                         num_conv_branches=4)
        self.conv1x1 = MobileOneBlock(in_channels=self.in_channels,
                                         out_channels=self.out_channels,
                                         kernel_size=1,
                                         stride=1,
                                         padding=0,
    ...
    def switch_to_deploy(self):
        ...
        self.conv3x3.reparameterize()
        self.conv1x1.reparameterize()
