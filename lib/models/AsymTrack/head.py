...
from lib.models.AsymTrack.backbone import FrozenBatchNorm2d
...
def build_box_head(cfg):
    if cfg.MODEL.HEAD_TYPE == "MLP":
        ...
        if cfg.MODEL.NECK.TYPE in ["LINEAR", "FPN", "UPSAMPLE", "FB", "MAXF", "MAXMINF", "MAXMIDF", "MINMIDF", "MIDF",
                                   "MINF"]:
            if 'efficientmod' in cfg.MODEL.BACKBONE.TYPE.lower():
                stride = stride
            else:
                stride = stride // (cfg.MODEL.NECK.NUM_LAYERS * cfg.MODEL.NECK.STRIDE)
        elif cfg.MODEL.NECK.TYPE in ["None"]:
            stride = stride // 2

        if cfg.MODEL.NECK.TYPE in ["FPN"]:
