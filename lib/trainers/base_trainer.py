        ...
        for i in range(num_tries):
            try:
                ...
                for epoch in range(self.epoch + 1, max_epochs + 1):
                    ...
                    # jiawen: set save epochs for saving disk space
                    if epoch in [int(max_epochs/3), int(2*max_epochs/3), int(5*max_epochs/6),
                                 max_epochs - 9, max_epochs - 8, max_epochs - 7, max_epochs - 6, max_epochs-5,
                                 max_epochs-4, max_epochs-3, max_epochs-2, max_epochs-1, max_epochs]:
                        if self._checkpoint_dir:
                            if self.settings.local_rank in [-1, 0]:
                                self.save_checkpoint()
                    toc = time.time()
                    print("This epoch costs {:.1f} min".format((toc - tic)/60))
            except:
